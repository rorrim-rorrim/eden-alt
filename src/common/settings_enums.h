...
#define ENUM(NAME, ...)                                                                            \
    ...
    template <>                                                                                    \
    inline std::vector<std::pair<std::string_view, NAME>>                                          \
    EnumMetadata<NAME>::Canonicalizations() {                                                      \
    ...
    template <>                                                                                    \
    inline NAME EnumMetadata<NAME>::GetFirst() {                                                   \
        return NAME::PP_HEAD(__VA_ARGS__);                                                         \
    }                                                                                              \
    template <>                                                                                    \
    inline NAME EnumMetadata<NAME>::GetLast() {                                                    \
...
template <>
inline std::vector<std::pair<std::string_view, AudioEngine>>
...
/// @brief This is just a sufficiently large number that is more than the number of other enums
/// declared here
template <>
...
ENUM(ResolutionSetup, Res1_4X, Res1_2X, Res3_4X, Res1X, Res5_4X, Res3_2X, Res2X, Res3X, Res4X,
     Res5X, Res6X, Res7X, Res8X);
ENUM(ScalingFilter, NearestNeighbor, Bilinear, Bicubic, Gaussian, Lanczos, ScaleForce, Fsr, Area,
     ZeroTangent, BSpline, Mitchell, Spline1, Mmpx, MaxEnum);
