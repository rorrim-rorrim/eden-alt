...
bool TranslatorVisitor::AXFlag() {
    ...
    const IR::U32 new_c =
        ir.And(ir.AndNot(c, ir.LogicalShiftLeft(v, ir.Imm8(1))), ir.Imm32(0x20000000));
...
bool TranslatorVisitor::XAFlag() {
    ...
    const IR::U32 new_n =
        ir.And(ir.LogicalShiftLeft(not_c, ir.Imm8(2)), ir.LogicalShiftLeft(not_z, ir.Imm8(1)));
    const IR::U32 new_z = ir.And(z, ir.LogicalShiftLeft(c, ir.Imm8(1)));
    const IR::U32 new_c = ir.Or(c, ir.LogicalShiftRight(z, ir.Imm8(1)));
    const IR::U32 new_v =
        ir.And(ir.LogicalShiftRight(not_c, ir.Imm8(1)), ir.LogicalShiftRight(z, ir.Imm8(2)));
