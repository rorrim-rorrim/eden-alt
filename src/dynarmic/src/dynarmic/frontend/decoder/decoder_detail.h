...
#pragma warning(disable : 4800) // forcing value to bool 'true' or 'false' (performance warning)
    ...
    struct VisitorCaller<ReturnType (V::*)(Args...)> {
        ...
        static constexpr auto Make(std::integer_sequence<size_t, iota...>,
            ...
            static_assert(std::is_same_v<visitor_type, V>,
    ...
    struct VisitorCaller<ReturnType (V::*)(Args...) const> {
        ...
        static constexpr auto Make(std::integer_sequence<size_t, iota...>,
                                   ...
                                   const std::array<opcode_type, sizeof...(iota)> arg_masks,
                                   const std::array<size_t, sizeof...(iota)> arg_shifts) {
            static_assert(std::is_same_v<visitor_type, const V>,
                          "Member function is not from Matcher's Visitor");
...
#pragma warning(pop)
    ...
    static constexpr auto GetMatcher(F fn) {
        ...
        const auto proxy_fn = VisitorCaller<F>::Make(std::make_index_sequence<args_count>(), fn,
                                                     arg_masks, arg_shifts);
        return MatcherT(mask, expect, proxy_fn);
    }
};

#define DYNARMIC_DECODER_GET_MATCHER(MatcherT, fn, name, bitstring)                                \
    Decoder::detail::detail<MatcherT<V>>::template GetMatcher<bitstring>(&V::fn)
