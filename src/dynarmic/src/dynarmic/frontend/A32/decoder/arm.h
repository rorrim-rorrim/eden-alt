...
#define INST(fn, name, bitstring)                                                                  \
    DYNARMIC_DECODER_GET_MATCHER(ArmMatcher, fn, name,                                             \
...
std::optional<std::reference_wrapper<const ArmMatcher<V>>> DecodeArm(u32 instruction) noexcept {
    ...
    return iter != subtable.end()
               ? std::optional<std::reference_wrapper<const ArmMatcher<V>>>(*iter)
               : std::nullopt;
}
...
#define INST(fn, name, bitstring)                                                                  \
    {name, DYNARMIC_DECODER_GET_MATCHER(ArmMatcher, fn, name,                                      \
                                        Decoder::detail::StringToArray<32>(bitstring))},
#include "./arm.inc"
#undef INST
    };
