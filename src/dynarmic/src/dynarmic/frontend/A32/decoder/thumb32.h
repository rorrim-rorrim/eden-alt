...
#define INST(fn, name, bitstring)                                                                  \
    DYNARMIC_DECODER_GET_MATCHER(Thumb32Matcher, fn, name,                                         \
...
#undef INST
    ...
    return iter != table.end()
               ? std::optional<std::reference_wrapper<const Thumb32Matcher<V>>>(*iter)
               : std::nullopt;
}
...
#define INST(fn, name, bitstring)                                                                  \
    {name, DYNARMIC_DECODER_GET_MATCHER(Thumb32Matcher, fn, name,                                  \
                                        Decoder::detail::StringToArray<32>(bitstring))},
#include "./thumb32.inc"
#undef INST
    };
