...
#define INST(fn, name, bitstring)                                                                  \
    DYNARMIC_DECODER_GET_MATCHER(Thumb16Matcher, fn, name,                                         \
                                 Decoder::detail::StringToArray<16>(bitstring)),
#include "./thumb16.inc"
#undef INST
    };
    ...
    return iter != table.end()
               ? std::optional<std::reference_wrapper<const Thumb16Matcher<V>>>(*iter)
               : std::nullopt;
}
...
#define INST(fn, name, bitstring)                                                                  \
    {name, DYNARMIC_DECODER_GET_MATCHER(Thumb16Matcher, fn, name,                                  \
                                        Decoder::detail::StringToArray<16>(bitstring))},
#include "./thumb16.inc"
#undef INST
    };
