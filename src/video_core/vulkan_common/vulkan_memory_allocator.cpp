...
static VkBuffer GetVkHandleFromBuffer(const T& buf) {
    if constexpr (requires { static_cast<VkBuffer>(buf); }) {
        return static_cast<VkBuffer>(buf);
                             ...
                             { buf.GetHandle() } -> std::convertible_to<VkBuffer>;
                         }) {
        return buf.GetHandle();
    } else if constexpr (requires {
                             { buf.Handle() } -> std::convertible_to<VkBuffer>;
                         }) {
        return buf.Handle();
    } else if constexpr (requires {
                             { buf.vk_handle() } -> std::convertible_to<VkBuffer>;
                         }) {
...
MemoryCommit::MemoryCommit(VmaAllocator alloc, VmaAllocation a,
    ...
    : allocator{alloc}, allocation{a}, memory{info.deviceMemory}, offset{info.offset},
      size{info.size}, mapped_ptr{info.pMappedData} {}
...
MemoryCommit::MemoryCommit(MemoryCommit&& rhs) noexcept
    : allocator{std::exchange(rhs.allocator, nullptr)},
      ...
      memory{std::exchange(rhs.memory, VK_NULL_HANDLE)}, offset{std::exchange(rhs.offset, 0)},
      size{std::exchange(rhs.size, 0)}, mapped_ptr{std::exchange(rhs.mapped_ptr, nullptr)} {}
...
std::span<u8> MemoryCommit::Map() {
    if (!allocation)
        return {};
    if (!mapped_ptr) {
        if (vmaMapMemory(allocator, allocation, &mapped_ptr) != VK_SUCCESS)
            return {};
    }
    const size_t n =
        static_cast<size_t>(std::min<VkDeviceSize>(size, (std::numeric_limits<size_t>::max)()));
...
std::span<const u8> MemoryCommit::Map() const {
    ...
    if (!mapped_ptr) {
        void* p = nullptr;
        if (vmaMapMemory(allocator, allocation, &p) != VK_SUCCESS)
            return {};
...
MemoryAllocator::MemoryAllocator(const Device& device_)
    ...
    // Preserve the previous "RenderDoc small heap" trimming behavior that we had in original vma
    // minus the heap bug
